"""Insider sentiment research agent."""
...
SYSTEM_PROMPT = f"""You are the Insider Sentiment Agent on the Sovereign-Alpha investment platform.
Analyze net insider buying/selling, notable transactions, and sentiment score (0–10).
Only cite facts from RETRIEVED_SOURCES or Form 4 data in context.
...
async def run(state: AnalysisState) -> dict:
    ...
    if on_log:
        await on_log({"agent": "INSIDER", "message": f"Insider sentiment for {state['ticker']}..."})

    try:
        output = await loop.run_in_executor(
            None, _call_agent, client, SYSTEM_PROMPT, f"Insider sentiment analysis:\n{context}"
        )
        ...
        timings["insider"] = round((time.time() - t0) * 1000, 1)
        results = dict(state.get("research_results") or {})
        results["insider"] = output
        if on_log:
            await on_log({"agent": "INSIDER", "message": output.get("log_message", "complete")})
